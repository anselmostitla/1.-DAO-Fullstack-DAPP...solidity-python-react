...
contract DominionDAO is AccessControl, ReentrancyGuard {
    bytes32 private immutable CONTRIBUTOR_ROLE = keccak256("CONTRIBUTOR");
    bytes32 private immutable STAKEHOLDER_ROLE = keccak256("STAKEHOLDER");
    uint256 immutable MIN_STAKEHOLDER_CONTRIBUTION = 10 wei;
    // uint32 immutable MIN_VOTE_DURATION = 1 weeks;
    uint32 immutable MIN_VOTE_DURATION = 60 minutes;
    ...
    struct VotedStruct {
        address voter;
        uint256 timestamp;
        bool choosen;   // if already has voted or not
    }
    ...
    function contribute() public payable {
        require(msg.value > 0 ether, "Contributting zero is not allowed.");
